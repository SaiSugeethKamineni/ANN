...
class ANN:
    ...
    def mini_batch(self, input, target, batch_size):
        input_batches = []
        target_batches = []
        for step in range(0, input.shape[0] - batch_size + 1, batch_size):
            batch = slice(step, step + batch_size)
            input_batches.append(input[batch])
            target_batches.append(target[batch])
    ...
    def forward_prop(self, input):
        ...
        for i in range(self.num_hidden_layers+1):
            ...
            if i < self.num_hidden_layers:
                ...
                prop.append(temp)
        return (np.array(v_vals), np.array(act_vals), np.array(prop))

    def backward_prop(self, v_vals, act_vals, prop, error, learn_rate):
        delWeights = []
        del_j = logsitic_derv(v_vals[-1]) * error
    ...
    def train(self, learn_rate, epochs, batch_size):
        ...
        for epoch in range(epochs):
            count = 0
            batches_tuple = self.mini_batch(
                train_norminputs, train_normtargets, batch_size)
            input_batches = batches_tuple[0]
            target_batches = batches_tuple[1]
            cum_cost = 0
            for i, j in zip(input_batches, target_batches):
                state = self.forward_prop(i[0])
                cum_error = 0
                v_vals_sum = np.zeros(state[0].shape)
                act_vals_sum = np.zeros(state[1].shape)
                prop_sum = np.zeros(state[2].shape)
                for k in range(batch_size):
                    state = self.forward_prop(i[k])
                    cum_error += np.asscalar(j[k] - state[1][-1])

                    v_vals_sum = np.add(state[0], v_vals_sum)
                    act_vals_sum = state[1] + act_vals_sum
                    prop_sum = state[2] + prop_sum
                v_val_avg = v_vals_sum / batch_size
                act_val_avg = act_vals_sum / batch_size
                prop_avg = prop_sum / batch_size
                avg_error = cum_error / batch_size
                del_w = self.backward_prop(
                    v_val_avg, act_val_avg, prop_avg, avg_error, learn_rate)
                for i in range(len(self.weights)):
                    self.weights[i] += del_w[i]
                count += 1
                cum_cost += (avg_error**2 / 2)
...
if __name__ == "__main__":
    data_split("data.csv")
    ann = ANN(ARCH, ACTFUN)
    ann.train(LEARNING_RATE, EPOCHS, BATCH_SIZE)
